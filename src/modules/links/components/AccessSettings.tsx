...
export function AccessSettings({ link }: AccessSettingsProps) {
  ...
  const [approvalMode, setApprovalMode] = useState<
    "manual" | "auto" | "domain"
  >(link.approvalMode);
  const [approvedDomain, setApprovedDomain] = useState<string>(
    link.approvedDomain || ""
  );
  ...
  const handleSubmit = () => {
    ...
    if (approvalMode === "domain") {
      if (!approvedDomain) {
        toast.error(
          "Approved domain is required when domain approval mode is selected"
        );
        return;
      }
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="space-y-2">
          ...
          <Select
            value={visibility}
            onValueChange={(value: "public" | "request" | "private") =>
              setVisibility(value)
            }
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="request">
                Request (Users must request access)
              </SelectItem>
        ...
        {visibility === "request" && (
          <div className="space-y-2">
            ...
            <Select
              value={approvalMode}
              onValueChange={(value: "manual" | "auto" | "domain") =>
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="manual">
                  Manual (You approve each request)
                </SelectItem>
                <SelectItem value="auto">
                  Auto (All requests are automatically approved)
                </SelectItem>
                <SelectItem value="domain">
                  Domain (Approve based on email domain)
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
